...
- **chrome.debugger auto-reconnect with exponential backoff.** `chrome.debugger.onDetach` schedules a reconnect via `chrome.alarms.create('piChromeReconnect', {periodInMinutes: 1})` and a `reconnectAttempts: Map<tabId, {count, nextDelayMs, scheduledAt}>` with schedule 250ms → 30s and ±25% jitter. `canceled_by_user` branch is preserved (no fight with the user); attach waits for `chrome.tabs.get(tabId).status === 'complete'`; backoff ceiling 10s so it never races the bridge `/next` ping. `/chrome doctor` surfaces the snapshot.
- **Bridge `/health` route (companion probes first).** New additive HTTP route on the bridge returns the same shape as `/status` with `cache-control: no-store`; companion `probeBridge` calls `/health` first with a 750ms budget and falls back to `/status`. Wire-protocol unchanged; capability flag `health: true` added to `tab.version`.
- **chrome_click auto-fallback chain.** New `chromeClickViaChain(tabId, params, baseResolved)` runs `uid-CDP → selector-CDP → uid-DOM → selector-DOM → native el.click()`; each link only fires on a stale-CDP error (`Debugger is not attached | Detached while | Target closed | No tab with id`); the existing `cdp()` wrapper's one-shot reattach runs BEFORE the chain so self-heal is preserved. Result gains an additive `syntheticFallback` field: `cdp-only | dom-click | react-prog`. Older pi sessions ignore the unknown field.
- **chrome_set_native_value.** First-class tool encapsulating `reactCompatFill` (native value descriptor setter + `InputEvent('input')` + `Event('change')`). JS-only, no keyboard simulation. Tool description warns against use on IME / `onKeyDown` / `onCompositionStart` inputs. Reuses the inline duplication pattern from `chromeInputFill`; refactor of the duplicate is out of scope for this release.
- **chrome_set_value.** First-class tool that probes `detectReactControlled` then routes through `reactCompatFill` (when React-controlled) or the CDP triple-click + delete + `typeTextInTab` path (otherwise). Optional `verifyExpr` polling, `verifyRetries` cap 5 default 2. Independent code path from `chrome_set_native_value` — never call internally.
- **chrome_click_retry.** Outer-loop retries around the chain. Schema: `retries` cap 5 default 0 (zero = single click, opt-in to retry behavior), `delayMs` cap 5000 default 250, `backoff` `linear` (default) | `exponential`. Tool description explicitly warns against retrying destructive clicks (Send / Delete).
- **chrome_session_check + login-page signatures.** New `page.sessionCheck` wire action returns `{url, title, isLoginPage, matchedSignature, suggestedAction, documentReady}`. Five-entry signature table (Microsoft SSO, Google SSO, GitHub Login, Auth0, Okta) matches on URL + title + first 200 chars of `innerText`. CSP-blocked pages degrade to title-only matching with `cspBlocked: true`; privacy-by-design (no form values exfiltrated).
- **Auto-relogin helper (`chrome_credentials_*`).** New tools `chrome_credentials_list | chrome_credentials_add | chrome_credentials_remove | chrome_credentials_fill` backed by an encrypted `~/.pi-chrome/credentials.json` store (chmod 600, AES-256-GCM scrypt key). `keytar` / `libsecret` / `DPAPI` are the preferred passphrase store; passphrase file fallback when none is available. Wire action `credentials.fill` is gated by `chrome_session_check` (auto-relogin only fires on real login pages) and by `hostnameOf(currentTab.url) === alias.host` (cross-host refusal even on substring match). Rate-limited to 5 attempts per alias per hour. MFA / CAPTCHA paths short-circuit with a `ctx.ui.notify` and never attempt to solve. Audit log at `~/.pi-chrome/credentials.audit.log` (chmod 600).

## 0.17.0 — 2026-09-17

- **uid-preferred `chrome_click`.** Precedence is now `uid > selector > x,y`. Bare `x,y` clicks warn when no fresh snapshot exists (viewport may have shifted); pass `allowCoordFallback=true` to suppress. `chrome_snapshot` refreshes the freshness timestamp.
- **Animation-aware `chrome_wait_for`.** Adds `waitForVisible` (default true: skip hidden selectors), `waitForStable` (require rect+opacity unchanged for N ms), `waitForSelectorCount` (succeed only at ≥ N matches), and `includeSnapshot: "auto"` (snapshot only when the wait found something). Fixes modal-open `offsetParent: null` and react-select menu race in one param set.
- **Bootstrap-tab-toggle detector.** `chrome_click` now auto-dispatches a synthetic bubbling click on `a.nav-link[data-rb-event-key]` / `[data-bs-toggle="tab|pill"]` and skips CDP mousePressed. Plain anchors and buttons fall through unchanged.
- **`chrome_fill` React-Compat.** Detects `__reactFiber$*` / `__reactProps$*` and applies the native value setter so React-controlled inputs commit to state. CDP-typing and DOM-fallback paths share the helper.
- **Save-Bubble retry.** `chrome_click` accepts `verifyExpr`, `verifyAfterMs`, `verifyRetries`. Re-clicks up to N times when the probe stays falsy, with ~150ms jitter. Opt-in, default off.
- **Doc clarity.** `chrome_fill` description now uses `text=` (not `value`); `chrome_wait_for` clarifies `value` is shared between selector and expression kinds.

## 0.16.1 — 2026-09-16

- **Auto-resolve `targetId` for `page.*` tools.** Pi now remembers the most recently observed tab id (`chrome_tab list` refresh, `chrome_tab new/activate/group` result, `chrome_navigate` result) and uses it as the default for `page.snapshot`, `page.evaluate`, `page.click`, `page.type`, `page.fill`, `page.key`, `page.waitFor`, `page.screenshot`, `page.hover`, `page.drag`, `page.tap`, `page.scroll`, `page.upload`, `page.console.list`, `page.network.list`, `page.network.get` when no `targetId`/`urlIncludes`/`titleIncludes` is supplied. Explicit targeting still wins. The cache clears on `session_start` so stale ids from a previous omp run never route to the wrong tab. No wire-protocol change; the browser-extension is untouched.
